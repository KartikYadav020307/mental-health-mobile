...
export default function AuthScreen() {
  ...
  const handleGuest = async () => {
    ...
    try {
      ...
      if (data.user) {
        useUserStore.getState().setSession(data.user.id, null, true);
      }
    } catch (error: any) {
      Alert.alert('Guest Login Error', error.message || 'An unexpected error occurred.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (data.user) {
        useUserStore.getState().setSession(data.user.id, data.user.email || null, false);
      }
    } catch (error: any) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.user) {
        // Fetch the user profile first to see if they completed onboarding already
            ...
            {mode === 'landing' ? (
              <View style={styles.landingContainer}>
                ...
                <View style={styles.landingBottom}>
                  <TouchableOpacity style={[styles.primaryButton, loading && { opacity: 0.6 }]} onPress={() => setMode('signup')} activeOpacity={0.8} disabled={loading}>
                    <Text style={styles.primaryButtonText}>GET STARTED</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.secondaryButton, loading && { opacity: 0.6 }]} onPress={() => setMode('login')} activeOpacity={0.8} disabled={loading}>
                    <Text style={styles.secondaryButtonText}>I ALREADY HAVE AN ACCOUNT</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.guestButton, loading && { opacity: 0.6 }]} onPress={handleGuest} disabled={loading}>
              ...
              <View style={styles.formContainer}>
                <View style={styles.formHeader}>
                  <TouchableOpacity onPress={() => setMode('landing')} style={styles.backButton} disabled={loading}>
                ...
                <View style={styles.formContent}>
                  ...
                  <TouchableOpacity style={[styles.socialButton, loading && { opacity: 0.6 }]} activeOpacity={0.8} disabled={loading}>
                    <Text style={styles.socialButtonText}>CONTINUE WITH GOOGLE</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.appleSocialButton, loading && { opacity: 0.6 }]} activeOpacity={0.8} disabled={loading}>
...
const styles = StyleSheet.create({
  ...
  appleSocialButton: {
    ...
    borderLeftColor: '#1A1F2B',
    borderRightColor: '#1A1F2B',
